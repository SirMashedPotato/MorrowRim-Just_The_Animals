...
namespace MorrowRim
{
	public class IncidentWorker_AlbinoGuar : IncidentWorker
	{
		protected override bool CanFireNowSub(IncidentParms parms)
		{
			if (!base.CanFireNowSub(parms) || !ModSettings_Utility.MorrowRim_SettingEnableAlbinoGuarEvent())
			{
				return false;
			}
		...
		protected override bool TryExecuteWorker(IncidentParms parms)
		{
			...
			Verse.PawnKindDef p = PawnKindDefOf.MorrowRim_AlbinoGuar;
			int num = 1;
			...
			for (int i = 0; i < num; i++)
			{
				...
				pawn = PawnGenerator.GeneratePawn(p, null);
			...
			Find.LetterStack.ReceiveLetter("LetterLabelAlbinoGuarPasses".Translate(p.label).CapitalizeFirst(), "LetterAlbinoGuarPasses".Translate(p.label), LetterDefOf.PositiveEvent, pawn, null, null);
			return true;
		}
		//
